...
RESPONSE FORMAT — ONLY valid JSON, NO markdown, NO code fences. ALL fields required:
{
  ...
  // ── RC-8 FIX (v2): Server-side stage_update_hint synthesis ────────────────
  // ROOT CAUSE (confirmed from live thread thrmu1c2ad83659kr): GPT sets action=none
  // and omits stage_update_hint even after generating rules/workflow/compliance in
  // its message text.  GPT also uses current_stage to mean the NEXT stage (e.g. it
  // lists Stage 3 rules but returns current_stage=4), making the previous v1 fix
  // unreliable — v1 checked `stage === 3` which was false when GPT returned
  // current_stage=4 after the rules turn.
  //
  // v2 fix: detect stage completion purely from message TEXT patterns — these are
  // the most reliable signals since GPT consistently uses them even when it violates
  // the action/hint schema.  current_stage and ui_events are used as supplementary
  // signals only.  The conversation history (messages[]) provides additional context.
  if (!aiReply.stage_update_hint) {
    const action = (aiReply.action || 'none').toLowerCase()
    const msgRaw = aiReply.message || ''
    const msgLow = msgRaw.toLowerCase().replace(/<[^>]+>/g, ' ')
    ...
    // ── Derive current pge_stage from conversation history ──────────────────
    // We track how far the conversation has actually progressed rather than trusting
    // aiReply.current_stage (which GPT often advances prematurely).
    // Look at the last 6 assistant messages (now including the one just pushed).
    const recentAssistant = messages
      .filter((m: any) => m.role === 'assistant')
      .slice(-6)
      .map((m: any) => (m.content || '').toLowerCase())

    // The stage that was CONFIRMED this turn is what we need to save.
    // Priority: check message text first (most reliable), then events, then action.

    // ── Stage 2 completion ──────────────────────────────────────────────────
    // Signals: "stage 2 complete" OR transitioning to Stage 3 eligibility rules question
    const stage2JustCompleted =
      msgLow.includes('stage 2 complete') ||
      // GPT sends the Stage-2-done / Stage-3-intro message
      (msgLow.includes('stage 3') && msgLow.includes('eligibility rule')) ||
      (msgLow.includes('gsas minimum') && (msgLow.includes('70') || msgLow.includes('75'))) ||
      (isStageUpdateAction && hasSetField)

    // ── Stage 3 completion ──────────────────────────────────────────────────
    // Signals: GPT lists ≥3 rules in text, OR mentions "14 rules" / "17 rules" /
    // "eligibility rules" AND transitions to Stage 4 workflow question, OR emits add_rule events.
    // Key real-world pattern from live thread: "I'll generate the 14 eligibility rules
    // covering credit, collateral, ESG, income, nationality, and employment based on a GSAS..."
    const stage3JustCompleted =
      msgLow.includes('stage 3 complete') ||
      hasAddRule ||
      // GPT listed rules in prose and transitions to Stage 4
      (msgLow.includes('eligibility rule') && (msgLow.includes('stage 4') || msgLow.includes('workflow') || msgLow.includes('10-step'))) ||
      (msgLow.includes('eligibility rule') && msgLow.includes('dbr') && msgLow.includes('ltv')) ||
      // GPT says "14 rules" / "17 rules" — any number + rules in same message
      /\b(1[0-9]|[2-9])\s+eligibility rules?\b/i.test(msgRaw) ||
      (msgLow.includes('credit risk') && msgLow.includes('collateral') && msgLow.includes('esg') && msgLow.includes('rule')) ||
      (isStageUpdateAction && hasAddRule)

    // ── Stage 4 completion ──────────────────────────────────────────────────
    // Signals: GPT describes 10-step workflow and transitions to Stage 5.
    // Real pattern from live thread: "I'll configure the workflow to automate the first 5 steps.
    // Here is the proposed 10-step workflow: 1. Start 2. eKYC..."
    const stage4JustCompleted =
      msgLow.includes('stage 4 complete') ||
      hasSetWorkflow ||
      // GPT lists workflow steps and transitions to Stage 5
      (msgLow.includes('workflow') && (msgLow.includes('stage 5') || msgLow.includes('compliance'))) ||
      (msgLow.includes('10-step') || msgLow.includes('10 step')) ||
      // GPT lists specific numbered workflow steps
      (msgLow.includes('ekyc') && msgLow.includes('credit bureau') && msgLow.includes('workflow')) ||
      (msgLow.includes('automate') && msgLow.includes('step') && msgLow.includes('workflow')) ||
      (isStageUpdateAction && hasSetWorkflow)

    // ── Stage 5 completion ──────────────────────────────────────────────────
    // Signals: "stage 5 complete", compliance applied, or Basel III mentioned with IFRS9.
    // Real pattern from live thread: "✅ Stage 5 complete. Stage 6 — Portfolio Simulation."
    const stage5JustCompleted =
      msgLow.includes('stage 5 complete') ||
      msgLow.includes('compliance parameters applied') ||
      msgLow.includes('compliance classification applied') ||
      (msgLow.includes('basel iii') && msgLow.includes('ifrs9')) ||
      (msgLow.includes('stage 6') && msgLow.includes('simulation') && recentAssistant.some(m =>
        m.includes('compliance') || m.includes('shall i apply these compliance')
      )) ||
      (isStageUpdateAction && !hasAddRule && !hasSetWorkflow && !hasSetField)

    // ── Stage 6 completion ──────────────────────────────────────────────────
    // Signals: "all 6 stages complete", "confirm & publish", ready_to_confirm action.
    // Real pattern: "🚀 All 6 stages complete. Click Confirm & Publish to save."
    const stage6JustCompleted =
      action === 'ready_to_confirm' ||
      msgLow.includes('all 6 stages complete') ||
      msgLow.includes('confirm & publish') ||
      msgLow.includes('confirm and publish') ||
      msgLow.includes('click confirm') ||
      (msgLow.includes('stage 6') && msgLow.includes('simulation') && msgLow.includes('portfolio'))

    // ── Synthesize hint for the first matching stage ─────────────────────────
    // Priority order: stage 6 > stage 5 > stage 4 > stage 3 > stage 2
    // (highest stage wins to avoid double-firing e.g. if Stage 5 complete msg also mentions Stage 3)
    // But also guard against false positives: only fire when the conversation has
    // progressed past the prerequisite stages (check recent assistant messages).
    const prevMsgsHaveStage2 = recentAssistant.some(m => m.includes('stage 2 complete') || m.includes('eligibility rule') || m.includes('gsas minimum'))
    const prevMsgsHaveStage3 = recentAssistant.some(m =>
      m.includes('stage 3 complete') || m.includes('10-step') || m.includes('ekyc') || m.includes('eligibility rule')
    )
    const prevMsgsHaveStage4 = recentAssistant.some(m =>
      m.includes('stage 4 complete') || m.includes('compliance') || m.includes('stage 5')
    )
    ...
    } else if (stage5JustCompleted && prevMsgsHaveStage4) {
    ...
    } else if (stage4JustCompleted && prevMsgsHaveStage3) {
    ...
    } else if (stage3JustCompleted && prevMsgsHaveStage2) {
      const hintRules = events.filter((e: any) => e.type === 'add_rule' && e.rule).map((e: any) => e.rule)
      aiReply.stage_update_hint = { stage: 3, rules: hintRules }
      if (!isStageUpdateAction) aiReply.action = 'stage_update'

    } else if (stage2JustCompleted) {
      const hintFields: Record<string, any> = {}
      for (const e of events) {
        if (e.type === 'set_field' && e.field && e.value != null) hintFields[e.field] = e.value
      }
